/**
 * Pushes YANGO_TOKEN_STAR_TAXI_POINT from .env.local into Vercel (production, preview, development).
 * Run: npm run vercel:sync-yango-star-taxi-env
 * Auth: VERCEL_TOKEN or VERCEL_OIDC_TOKEN in .env.local.
 */
...
const KEY = "YANGO_TOKEN_STAR_TAXI_POINT";
...
if (!vercelToken && !vercelOidc) {
  console.error("Missing VERCEL_TOKEN or VERCEL_OIDC_TOKEN in .env.local or process.env");
  process.exit(1);
}
