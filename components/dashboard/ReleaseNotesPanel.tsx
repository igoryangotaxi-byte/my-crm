...
const releaseItems: ReleaseItem[] = [
  ...
  },
  {
    ...
    notes:
      "App version 0.2.72. 3CX and Astradial dial independently — no cross-fallback. With NEXT_PUBLIC_TELEPHONY_ENABLED, driver cards show both 3CX and Astradial buttons for A/B. Astradial page dials Astradial only; Call Center / default Dial stays on 3CX. Nav: Astradial via salesAstradial or salesCallCenter (permissions v18).",
  },
  {
    date: "2026-09-20",
  ...
  },
  {
    ...
    notes:
      "App version 0.2.58. Two things that made the assistant feel like a command list rather than an assistant. First, client identification: crm.lookup resolves whatever the user says — lead id, client id, Corp Client ID, phone in any format (0521234567 finds +972 52-123-4567), email, company or person name — and returns the whole picture in one call: pipeline status and how long it has been there, owning manager, monthly potential and trips, pricing and contract, all contacts with titles and phones, open tasks, the last notes, and the signed-client card with account and sales manager. An ambiguous query comes back with the candidate list instead of a silent guess, and search now indexes record ids, Corp Client IDs, contract and registration numbers, and bare phone digits, so ⌘K search in the UI finds them too. The system prompt tells the assistant to answer with those facts rather than “found 1 match”, and to never ask which identifier was meant before trying. Second, Telegram: sending to your own linked chat is a note to yourself, not an external send, so it goes out immediately with no confirmation card — messages to any other chat still ask, and email confirmation is unchanged. No schema changes. (3f353011)",
  },
  {
    date: "2026-08-14",
    title: "AI assistant runs the Tracker (0.2.57)",
    notes:
      "App version 0.2.57. The assistant can now work in the Tracker, which it previously could not touch at all — asking it for a queue with a ticket inside produced a personal task instead. Ten new tools cover the full loop: list and create queues, list, read, create, update, assign, comment on, archive and permanently delete tickets. Practical details that remove the follow-up questions: tracker.create_ticket creates the queue when it does not exist yet, so “create queue AI Test with ticket Ticket Test assigned to me” is a single call; queue and column names match case- and spacing-insensitively (“ai test” finds “AI Test”); assignees accept “me”, a staff name or a user id; and columns are addressed by name (Backlog, In Progress, Done) rather than ids. Tickets are resolved by id or by title, and an ambiguous title comes back with the candidate list instead of a silent guess. Security is unchanged in spirit: every tool requires the salesTracker page key plus the same Tracker role matrix the UI enforces (editBoard, editStatuses, createTickets, assignTickets, archiveTickets, deleteTickets), so a role that cannot create boards cannot create them through chat either. Archiving is a normal write; permanent deletion is risk 3 and always raises a confirmation card naming what will be lost. The system prompt now states that a “queue”, “board” or “очередь” means a Tracker queue and that a ticket is never a CRM or personal task. No schema changes. (0f47229a)",
  },
  {
    date: "2026-08-14",
    title: "AI assistant: one meeting counted once (0.2.56)",
    notes:
      "App version 0.2.56. Fixes the assistant reporting two meetings when the calendar holds one. A meeting booked by the assistant exists twice by design — as a Google Calendar event and as its sales_meetings mirror that the MySpace calendar reads — and calendar reads passed both copies to the model. Mirrors are now folded into their Google event by google_event_id, with a fallback match on identical instant and title for rows saved before the link existed, so calendar.get_events returns one entry per real meeting plus a true count. The same double counting silently inflated the calendar load score, raised conflict warnings against a meeting's own mirror, and doubled the tomorrow briefing — all corrected. Model configuration hardened: an empty OPENAI_MODEL in production was passed straight through to OpenAI (it defeated the ?? default), breaking assistant turns as well as Tariff Health Check and Transcripts decoupling suggestions. The assistant now reads its own AI_ASSISTANT_MODEL (default gpt-4.1) so the cheaper OPENAI_MODEL used for tariff analysis no longer plans its tool calls; leave both unset rather than empty. A turn that returns no text — empty reply or the round budget spent mid-plan — now reports the last tool result instead of leaving the drawer blank. No schema or permission changes. (6c56f5af)",
  },
  {
    date: "2026-08-14",
    title: "Appli AI executive assistant (0.2.55)",
    notes:
      "App version 0.2.55. Per-user AI copilot across Sales Operation: chat + push-to-talk voice (Alt+Space), conversation memory, and a tool gateway that runs everything as the signed-in user with RBAC, risk tiers, idempotency and an audit trail. Tools cover CRM search, lead status moves through the gated pipeline flow (crm.update_lead_status accepts \"In Progress\", \"в работе\", \"won\"), tasks, people workload, calendar (free/busy, slot finding, load score, create/update/cancel with conflict warnings), analytics, Gmail read/draft/send, reminders, briefings and Telegram. Everyday writes execute immediately; only sends, cancels and bulk actions raise a confirmation card. The system prompt carries the current date, so \"tomorrow\" resolves correctly, and calendar writes refuse past windows and send invites to attendees via sendUpdates=all. Google Calendar + Gmail now come from the main Google SSO login (combined offline scopes persisted per user), so no separate connect step. Telegram linking shows explicit state — code, one-tap deep link to the assistant bot, live polling and disconnect. New page key salesAiAssistant (Admin/SM/AM on, User off), permissions version 14. New APIs under /api/ai/** plus a daily /api/ai/cron/tick at 05:30 UTC (Vercel Hobby allows one cron run per day). SQL (run in Supabase): scripts/sql/supabase_ai_assistant.sql, also wired into npm run db:apply:sales-operation. Env (Vercel Production): OPENAI_API_KEY, OPENAI_MODEL, TELEGRAM_ASSISTANT_BOT_TOKEN, TELEGRAM_ASSISTANT_WEBHOOK_SECRET, optional AI_MONTHLY_BUDGET_USD and OpenClaw vars. Webhook: npm run telegram:set-assistant-webhook → https://applitaxi.space/api/ai/telegram/webhook. (f27ca691)",
  },
  {
    date: "2026-08-13",
  ...
  },
  {
    ...
    notes:
      "App version 0.2.50. Sales Operation is branded Appli Taxi CRM. Communications, Price Calculator and Access management live inside the SO shell (/sales-operation/communications, /sales-operation/price-calculator, Settings → Access); old Main CRM routes redirect. Staff Main CRM sidebar is empty — login/OAuth land in Sales Operation. New interactive 3D Office at /sales-operation/office (R3F): rooms, Pipeline Wall stickers with Advance, walking team agents (Igor K, Lior, Igor R, Itay, Egor, Ido, Adam, Gal) with hover + live badges, and an in-office CRM workbench (briefing, filtered leads + stage Advance, complete tasks, meetings, analytics funnel, Lead Discovery summary). Classic/3D toggle in the header; graphics Low/High/Static. Deps: three, @react-three/fiber, @react-three/drei. Vendored Claw3D MIT NOTICE only (no OpenClaw gateway). Deck: Yango-Sales-Operations-CRM-3D-Office-0-2-50.pptx. (4d38173a)",
  },
  {
    date: "2026-07-26",
    title: "Lead Discovery for Israel B2B (0.2.49)",
    notes:
      "App version 0.2.49. New Sales Operation module at /sales-operation/lead-discovery: describe a segment in natural language → Groq interprets business types, cities, size and qualification rules (heuristic fallback on 429) → Google Places search + scoring → candidates pending approval before pipeline. One campaign runs at a time; Stop mid-run; Activate runs Find leads; daily target editable in Advanced; cron discovery-tick daily at 07:00 UTC (Vercel Hobby). Automation gains discovery triggers (lead discovered, qualification completed, daily target, email replied), condition gate, notify/sticker/email-sequence actions. Permission salesLeadDiscovery (v13). RU locale for Lead Discovery. SQL: supabase_sales_lead_discovery.sql + supabase_sales_lead_discovery_pending_approval.sql (apply in Supabase). Env: LEAD_DISCOVERY_ENABLED, GOOGLE_PLACES_API_KEY (or Maps), GROQ_API_KEY. Deck: Yango-Sales-Operations-Lead-Discovery-0-2-49.pptx. (bac95b7c)",
  },
  {
    date: "2026-07-23",
  ...
  {
    date: "2026-07-22",
    title: "B2B client profile + My Space Calendar (0.2.46)",
    notes:
      "App version 0.2.46. B2B Client Overview search filters the metrics table by name or corp_client_id. Clicking a client ensures a CRM profile (creates a minimal signed lead + sales_clients row when missing) and opens an amoCRM-style profile: contact + deal fields on the left, activity feed on the right, and Task / Note / Mail / Meeting composer bubbles. Tasks/notes sync into My Space. Calendar moves to a My Space sidebar subsection (/sales-operation/calendar) with colored events (meetings sky, lead tasks violet, personal tasks amber). Clicking an event opens a side drawer to edit fields and append timestamped notes; lead tasks assigned from the pipeline card appear on the assignee's calendar (default due ≈ +2h). Staff pickers (assignees, owners, filters) only list approved @appli.taxi platform users. Optional Connect Google Calendar syncs meetings. SQL: supabase_sales_client_activity.sql. APIs: /b2b-clients/ensure, /clients/[id]/activity, /meetings, /google/calendar/*. (c0f45a40)",
  },
  {
    date: "2026-07-20",
