...
| Source | **`main@cd0d5424`** | **Linked operator (this branch)** | **Unlinked / idle poll (this branch)** |
|--------|---------------------|-----------------------------------|----------------------------------------|
...
- **In-process cache TTL:** `AUTH_KV_SNAPSHOT_TTL_MS` (30 seconds) — coalesces `kv.get` for the **global snapshot object** only. Successful reads are cached; **KV errors are not cached** (read behavior matches `main` until fail-closed PR B).
- **`/api/auth` responses:** `Cache-Control: no-store`.
- **Saves:** managed Supabase Auth user **deletions** are skipped unless the request successfully loaded the KV snapshot; save returns **503 `PERMISSION_STORE_UNAVAILABLE`** instead.
