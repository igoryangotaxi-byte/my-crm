...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const notes = await listDriverLeadNotes(id);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!body?.body?.trim()) {
    return Response.json({ ok: false, error: "Note body is required." }, { status: 400 });
  }

  try {
    const note = await createDriverLeadNote(id, body.body, {
      userId: auth.user.id,
      name: auth.user.name,
    });
