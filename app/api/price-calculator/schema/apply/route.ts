...
export async function POST(request: Request) {
  ...
  const sqlPath = path.join(process.cwd(), "scripts/sql/supabase_driver_price_comparison.sql");
  ...
  try {
    ...
    return Response.json({
      ok: true,
      message:
        "Applied taxi_orders, mone_price_imports, mone_prices, driver_price_comparison_enriched.",
    });
  } catch (error) {
    return Response.json(
      {
