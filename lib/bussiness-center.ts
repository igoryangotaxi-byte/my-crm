...
export type BussinessCenterTopBucket = {
  key: string;
  label: string;
  spend: number;
  rides: number;
...
export type BussinessCenterPayload = {
  ...
  topUsers: BussinessCenterTopBucket[];
  topDepartments: BussinessCenterTopBucket[];
...
export function normalizeDateRange(input: { since?: unknown; till?: unknown }) {
  const now = new Date();
...
export async function buildBussinessCenterPayload(input: {
  tokenLabel: string;
  clientId: string;
  since: string;
  till: string;
  maxOrders: number;
}): Promise<BussinessCenterPayload> {
  ...
  while (hasMore && rows.length < input.maxOrders) {
    const chunk = await pullB2BOrdersRows({
      since: input.since,
      till: input.till,
      startCursors: cursors,
      targetNewCount: Math.min(100, input.maxOrders - rows.length),
      excludeKeys,
      excludeScheduling: true,
      scope: { tokenLabel: input.tokenLabel, clientId: input.clientId },
    });
  ...
  const topUsersMap = new Map<string, BussinessCenterTopBucket>();
  const topDepartmentsMap = new Map<string, BussinessCenterTopBucket>();
      ...
      batch.map(async (row) => {
        try {
          const details = await getB2BOrderDetails({
            ...
            clientId: row.clientId ?? input.clientId,
            orderId: row.orderId,
          });
          return { row, details };
        } catch {
  ...
  return {
    range: { since: input.since, till: input.till },
    summary: {
      spendDay,
      spendWeek,
      spendMonth,
    ...
    topUsers: [...topUsersMap.values()].sort((a, b) => b.spend - a.spend).slice(0, 8),
    topDepartments: [...topDepartmentsMap.values()]
