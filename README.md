...
- **Transcripts MOT tariffs** — optional Supabase table `transcript_mot_tariffs`: run [`scripts/sql/supabase_transcript_mot_tariffs.sql`](scripts/sql/supabase_transcript_mot_tariffs.sql) in the Supabase SQL editor. If the table is missing or empty, the app uses the embedded 16-tariff catalog.
- **Transcripts past trip times** — Google Routes only accepts `departureTime` in the **future**. For trips at or before “now”, the app uses **traffic-unaware** routing (no historical congestion model).
